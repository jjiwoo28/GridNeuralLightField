...
from multi_model_each_yuv import GridNetworks
...
class nlf_grid_each_yuv(train):
    def __init__(self,args , mode = "uvst"):
        ...
        self.model = GridNetworks( grid_size= args.grid_n,D=args.mlp_depth,W=args.mlp_width,input_ch=args.mlp_width,input_dim=4)
    ...
    def train_step(self,args):
        ...
        for epoch in range(start_epoch,start_epoch+self.epoch_num+1):
            ...
            for network_index, network in enumerate(self.model.networks):
                ...
                self.uvst_train_gpu = torch.tensor(np.reshape(self.uvst_train[grid_h, grid_v,:,:,:,:],(-1,4))).float()
                ...
                for single_index,(single_network,optimizer,scheduler) in enumerate(zip(network.single_models,network.optimizers,network.schedulers )):
                    ...
                    for i, data_batch in enumerate(pbar):
                        ...
                        preds_color = single_network(inputs.cuda())
                        
                        
                        loss = 3000*torch.mean((preds_color - color[:,single_index]) * (preds_color - color[:,single_index]))
                        #print(f"loss : {single_index}  {loss}")
